...
export default function RemedioId() {
  ...
  const fetchById = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(`Remédio não encontrado (${res.status})`);
    ...
    <div className="min-h-screen bg-[#0d0d1a] text-[#e8e8f0] p-6">
      <div className="max-w-xl mx-auto">
        <div className="mb-8">
          ...
          <h1 className="text-2xl font-extrabold tracking-tight">Buscar Remédio por Código</h1>
          <p className="text-[#6b7090] text-sm mt-1">Retorna um Remédio específico pelo seu código.</p>
        </div>
        ...
        {remedio && (
          <div className="bg-[#161628] border border-[#3d7dca]/50 rounded-xl p-5">
            <div className="flex justify-between items-start mb-3">
              <span className="text-xs font-mono text-[#6b7090]">#{remedio.codigo}</span>
            </div>
            <h2 className="text-2xl font-extrabold mb-4">{remedio.nome}</h2>
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div className="bg-[#0d0d1a] rounded-lg p-3">
                <div className="text-xs text-[#6b7090] mb-1">Preço</div>
                <div className="font-bold">{remedio.preco}</div>
              </div>
              <div className="bg-[#0d0d1a] rounded-lg p-3">
                <div className="text-xs text-[#6b7090] mb-1">Fabricação</div>
                <div className="font-bold">{remedio.dataDeFabricacao}</div>
              </div>
              <div className="bg-[#0d0d1a] rounded-lg p-3">
                <div className="text-xs text-[#6b7090] mb-1">Validade</div>
                <div className="font-bold">{remedio.dataDeValidade}</div>
              </div>
            </div>
            <pre className="mt-4 bg-[#0d0d1a] rounded-lg p-3 text-xs text-green-400 font-mono overflow-auto">
              {JSON.stringify(remedio, null, 2)}
            </pre>
          </div>
        )}
      </div>
