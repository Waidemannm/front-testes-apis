    ...
    <div className="min-h-screen bg-[#0d0d1a] text-[#e8e8f0] p-6">
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          ...
          <h1 className="text-2xl font-extrabold tracking-tight">Listar Todos os Clientes</h1>
          <p className="text-[#6b7090] text-sm mt-1">Retorna todos os clientes cadastrados no banco.</p>
        </div>
            ...
            {clientes.map((c) => (
              <div key={c.codigo} className="bg-[#161628] border border-[#2a2a4a] rounded-xl p-4 hover:border-[#3d7dca] transition-colors">
                ...
                <div className="grid grid-cols-1 gap-1 text-xs text-[#6b7090]">
                  <span>CPF: <span className="text-[#e8e8f0]">{c.cpf}</span></span>
                  <span>Email: <span className="text-[#e8e8f0]">{c.Email}</span></span>
                  <span>Nascimento: <span className="text-[#e8e8f0]">{c.dataDeNascimento}</span></span>
                </div>
              </div>
            ))}
          </div>
