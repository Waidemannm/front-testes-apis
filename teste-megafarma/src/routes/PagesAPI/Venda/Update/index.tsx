...
const INITIAL = { dataDaVenda: "", codCliente: 0 };
...
export default function VendaUpdate() {
  ...
  const formatDate = (date: string) => {
    ...
    try {
      const body = {
        dataDaVenda: formatDate(form.dataDaVenda),
        codCliente: form.codCliente,
      };
      console.log(body);
      const res = await fetch(`${API}/venda/${codigo}`, {
        method: "PUT",
  ...
  const fields = [
    { name: "dataDaVenda",             label: "Data da venda",          placeholder: "",    type: "date" },
    { name: "codCliente",            label: "Código do cliente",         placeholder: "10",         type: "number" }
  ];
    ...
    <div className="min-h-screen bg-[#0d0d1a] text-[#e8e8f0] p-6">
      <div className="max-w-xl mx-auto">
        <div className="mb-8">
          ...
          <h1 className="text-2xl font-extrabold tracking-tight">Atualizar Venda</h1>
          <p className="text-[#6b7090] text-sm mt-1">Atualiza os dados de uma Venda existente.</p>
        </div>
        ...
        {response && (
          <div className="bg-green-500/10 border border-green-500/30 rounded-xl p-4">
            <div className="text-green-400 font-bold text-sm mb-2">✅ Venda atualizada com sucesso!</div>
