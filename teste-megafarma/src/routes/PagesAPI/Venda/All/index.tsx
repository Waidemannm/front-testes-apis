...
export default function VendaAll() {
    ...
    <div className="min-h-screen bg-[#0d0d1a] text-[#e8e8f0] p-6">
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          ...
          <h1 className="text-2xl font-extrabold tracking-tight">Listar Todos as Vendas</h1>
          <p className="text-[#6b7090] text-sm mt-1">Retorna todos as vendas cadastradas no banco.</p>
        </div>
            ...
            {vendas.map((c) => (
              <div key={c.codigo} className="bg-[#161628] border border-[#2a2a4a] rounded-xl p-4 hover:border-[#3d7dca] transition-colors">
                ...
                <div className="grid grid-cols-1 gap-1 text-xs text-[#6b7090]">
                  <span>Data da venda: <span className="text-[#e8e8f0]">{c.dataDaVenda}</span></span>
                  <span>código cliente: <span className="text-[#e8e8f0]">{c.codCliente}</span></span>
                </div>
              </div>
            ))}
          </div>
