...
export default function VendaId() {
  ...
  const fetchById = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(`Venda não encontrada (${res.status})`);
      ...
      <div className="max-w-xl mx-auto">
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-1">
            ...
            <code className="text-sm text-[#ffcb05] font-mono">/venda/{"{codigo}"}</code>
          </div>
          <h1 className="text-2xl font-extrabold tracking-tight">Buscar Venda por Código</h1>
          <p className="text-[#6b7090] text-sm mt-1">Retorna uma venda específica pelo seu código.</p>
        </div>
        ...
        {vendas && (
          <div className="bg-[#161628] border border-[#3d7dca]/50 rounded-xl p-5">
            <div className="flex justify-between items-start mb-3">
              <span className="text-xs font-mono text-[#6b7090]">#{vendas.codigo}</span>
            </div>
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div className="bg-[#0d0d1a] rounded-lg p-3">
                <div className="text-xs text-[#6b7090] mb-1">Preço</div>
                <div className="font-bold">{vendas.dataDaVenda}</div>
              </div>
              <div className="bg-[#0d0d1a] rounded-lg p-3">
                <div className="text-xs text-[#6b7090] mb-1">Fabricação</div>
                <div className="font-bold">{vendas.codCliente}</div>
              </div>
            </div>
