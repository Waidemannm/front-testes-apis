...
const INITIAL = {codVenda: 0, codRemedio: 0, quantidade: 0 };

export default function VendaUpdate() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const body = {
        codVenda: form.codVenda,
        codRemedio: form.codRemedio,
  ...
  const fields = [
    { name: "codVenda",             label: "Código da venda",          placeholder: "10",    type: "number" },
    { name: "codRemedio",            label: "Código do remédio",         placeholder: "10",         type: "number" },
    { name: "quantidade",            label: "Quantidade",         placeholder: "10",         type: "number" }
  ];
    ...
    <div className="min-h-screen bg-[#0d0d1a] text-[#e8e8f0] p-6">
      <div className="max-w-xl mx-auto">
        <div className="mb-8">
          ...
          <h1 className="text-2xl font-extrabold tracking-tight">Atualizar Item vendido</h1>
          <p className="text-[#6b7090] text-sm mt-1">Atualiza os dados de um item vendido existente.</p>
        </div>

        <div className="bg-[#161628] border border-[#2a2a4a] rounded-xl p-5 mb-4">
          <label className="block text-xs font-bold uppercase tracking-widest text-[#6b7090] mb-2">Código da Venda</label>
          <input
            type="number"
            placeholder="Ex: 1"
            value={codigo}
        ...
        {response && (
          <div className="bg-green-500/10 border border-green-500/30 rounded-xl p-4">
            <div className="text-green-400 font-bold text-sm mb-2">✅ Item vendido atualizado com sucesso!</div>
