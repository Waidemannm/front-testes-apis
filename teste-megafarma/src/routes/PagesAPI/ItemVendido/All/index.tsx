...
export default function ItemVendidoAll() {
  ...
  const fetchAll = async () => {
    ...
    try {
      const res = await fetch(`${API}/itemvendido`);
      ...
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-1">
            ...
            <code className="text-sm text-[#ffcb05] font-mono">/itemvendido</code>
          </div>
          <h1 className="text-2xl font-extrabold tracking-tight">Listar Todos os Itens vendidos</h1>
          <p className="text-[#6b7090] text-sm mt-1">Retorna todos os itens vendidos cadastrados no banco.</p>
        </div>
        ...
        {item.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {item.map((c) => (
              <div key={c.codVenda} className="bg-[#161628] border border-[#2a2a4a] rounded-xl p-4 hover:border-[#3d7dca] transition-colors">
                <div className="flex justify-between items-start mb-2">
                  <span className="text-xs font-mono text-[#6b7090]">#{c.codVenda}</span>
                </div>
                <div className="grid grid-cols-1 gap-1 text-xs text-[#6b7090]">
                  <span>Código remédio: <span className="text-[#e8e8f0]">{c.codRemdio}</span></span>
                  <span>Quantidade: <span className="text-[#e8e8f0]">{c.quantidade}</span></span>
                </div>
              </div>
            ))}
          </div>
